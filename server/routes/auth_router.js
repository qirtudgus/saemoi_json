...
router.post('/login', (req, res) => {
  ...
  db.query(loginQuery, [id, password], function (err, rows, fields) {
    ...
    // db조회값이 있을 시
      ...
      const dbProfile = rows[0].profile;
      ...
      if (dbPassword === userHashPassword) {
        ...
        const accessToken = jwt.sign(
          { userId: id, profile: dbProfile, TokenAuth: true },
          SECRET_TOKEN,
          {
            expiresIn: '10s',
          },
        ...
        const refreshToken = jwt.sign(
          { userId: id, profile: dbProfile, accessToken: accessToken },
          SECRET_TOKEN,
          { expiresIn: '1d' },
        );
...
// 비밀번호 찾기 시 이메일 인증
router.post('/passwordfind', (req, res) => {
  console.log('이메일');
  ...
  console.log(email);
  //비밀번호 찾기
  // 인증번호를 users 컬럼에 저장해버리면...악의적으로 계속 이메일을 전송하여 컬럼값을 바꿔 인증실패를 유도..
  //
  const main = async () => {
    ...
    let info = await transporter.sendMail({
      from: `"세모이" <${process.env.NODEMAILER_USER}>`,
      to: email,
      subject: '세모이 이메일 인증번호입니다.',
      text: `${randomNum}`,
    });
...
//이메일 인증 성공 후 비밀번호 생성
router.post('/changepasswordfind', (req, res) => {
  ...
  const sqlQuery = 'INSERT INTO users (id,password,salt) VALUES (?,?,?)';
  const changeQuery = 'UPDATE users SET password = ? , salt = ? WHERE id = ?';
  db.query(changeQuery, [hashPassword, salt, id]);
  console.log(`${id}님 비밀번호 찾기 완료`);
  res.send('비밀번호 찾기 완료');
});
