...
const Oliveyoung = () => {
    ...
    //json데이터를 받아올 api 주소
    const getApi = "https://sungtt.com/api/oliveApiData";
    //조회수를 전송할 api 주소
    const getView = "https://sungtt.com/api/oliveApiData/views";

    //테이블에 있는 데이터 가져오기
    function getOliveData(){
        return Axios.get("https://sungtt.com/api/oliveApiData");
    }
    ...
    function getNewOliveData() {
        return Axios.get(
                    ...
                    eventList = titleList.map((item,i) => ({...item,...dateList[i]})).map((item,i) => ({...item,...imgList[i]})).map((item,i) => ({...item,...linkList[i]}));
    ...
    useEffect(()=>{
        ...
        Axios.get("https://sungtt.com/api/oliveApiData").then(res => {setEventData([...res.data])})

        Axios.all([getOliveData(),getNewOliveData()])
        .then(Axios.spread(function (tableData, newData){

            const tableDataTitle = tableData.data.map(i => i.title);
            const newDataTitle = newData.map(i => i.title);
            const pareTitle = tableDataTitle.filter(i => !newDataTitle.includes((i)));
            ...
            if( pareTitle.length === 0){
                ...
                const compareNewData = newData.filter(
                    i => {
                        const title = i.title;
                ...
                // 없는 타이틀의 배열을 전송
                Axios.post("https://sungtt.com/api/oliveApiData/get", compareNewData);


                // 진행중인 이벤트엔없고, 기존엔 가지고있는 삭제해야할 배열 생성
                const delData = tableData.data.filter (
                    i => {
                        const title = i.title;
                ...
                // 종료된 타이틀의 배열을 전송
                Axios.post("https://sungtt.com/api/oliveApiData/end", delData);


                // 새로운 데이터를 받아와서, 렌더링
                Axios.get("https://sungtt.com/api/oliveApiData")
                .then((res) => {
    ...
    <>{ loading ? 
        <> <EventForm Data={eventData} setData={setEventData} getApi={getApi} getView={getView}/>
        <Comment brandName={brandName} getCommentApi="https://sungtt.com/api/oliveApiData/comment" postCommentApi="https://sungtt.com/api/oliveApiData/comment/post" /></> 
        :
        <Loading/>
   }
